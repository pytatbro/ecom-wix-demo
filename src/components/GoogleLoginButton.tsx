...
interface GoogleLoginButtonProps extends ButtonProps {
  returnIsRunning: (isRunning: boolean) => void;
  returnErrorString: (error: string) => void;
  returnSuccessString: (message: string) => void;
}
...
export default function GoogleLoginButton({
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        const response = await fetch(
          `${env.NEXT_PUBLIC_BASE_URL}/api/auth/external/google`,
          {
            method: "POST",
        ...
        if (!response.ok) {
          returnErrorString(res.error || errorString);
        } else {
          returnSuccessString("Login successful! Redirecting...");
    ...
    onError: (errorResponse) => {
      setLoading(false);
      console.error("Login with Google Error", errorResponse);
      returnErrorString(errorString);
      returnIsRunning(false);
    },
  });
